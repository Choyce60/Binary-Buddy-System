...
public class BinaryBuddySystemTree 
{
	...
	public static void main(String[] args) throws IOException 
	{
			...
			else if(inputChoiceNum == 2)
			{
				...
				Memory mainBlock = createMemoryBlock(memoryBlockSize);
				ArrayList<Memory> currentBank = new ArrayList<Memory>();
				currentBank.add(mainBlock);
	...
	private static Memory createMemoryBlock(int systemSize)
	{
	...
	private static ArrayList<Memory> split(ArrayList<Memory> editableList, int locationOfEdit)
	{
		if(editableList.get(locationOfEdit).getValue()>1)
		{
			...
			splitMem.setParent(editableList.get(locationOfEdit));
			splitMemTwo.setParent(editableList.get(locationOfEdit));
			editableList.remove(locationOfEdit);
			editableList.add(locationOfEdit, splitMem);
			editableList.add(locationOfEdit, splitMemTwo);
		...
		while(!flag)
		{
			...
			//{
				...
				//{
					...
					{
						if(!editableList.get(i).getActive()&&!editableList.get(i-1).getActive()&&editableList.get(i).getValue()==editableList.get(i-1).getValue()&&editableList.get(i).getParent().equals(editableList.get(i-1).getParent()))
						{
							flag=false;
							Memory marker = editableList.get(i).getParent();
							editableList.set(i, marker);
							editableList.remove(i-1);
	...
	private static void runFileInput() throws IOException
	{
		...
		if((line = br.readLine()) != null)
		{
			...
			ArrayList<Memory> currentBank =  new ArrayList<Memory>();
			currentBank.add(createMemoryBlock(memoryBlockSize));
